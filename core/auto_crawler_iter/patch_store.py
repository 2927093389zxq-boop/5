...
class PatchStore:
    ...
    def build_patch(self, original: str, variant: str, tag: str, meta: Dict) -> str:
        ...
        with open(patch_path, "w", encoding="utf-8") as f:
            f.writelines(diff)
        meta_path = os.path.join(self.patch_dir, f"{tag}.meta.json")
        meta["tag"] = tag
    ...
    def list_patches(self):
        ...
        for f in files:
            ...
            data.append({
                ...
                "mtime": time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime(os.path.getmtime(full)))
            })
    ...
    def apply_patch_direct(self, patch_path: str, production_file: str, variant_code: str):
        backup = production_file + f".bak.{int(time.time())}"
